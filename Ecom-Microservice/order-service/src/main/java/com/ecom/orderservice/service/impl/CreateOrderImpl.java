...
import com.ecom.orderservice.dto.InventoryReservationResult;
...
//        List<String> skuCodes = order1.stream().map(OrderLineItems::getSkuCode).toList();
        Map<String, Integer> skuCodes = order1.stream()
                .collect(Collectors.toMap(OrderLineItems::getSkuCode,OrderLineItems::getQuantity,Integer::sum));
...
//                .block();
        ...
        List<InventoryReservationResult> inventoryReservationResults = webClientBuilder
                .build()
                .post()
                .uri("http://inventory-service/api/inventory")
                .bodyValue(skuCodes)
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<List<InventoryReservationResult>>() {})
                .block();

//        boolean allProductsInStock = Arrays.stream(inventoryResponses).allMatch(InventoryResponse::getIsInStock);

        boolean allProductsInStock = inventoryReservationResults.stream().anyMatch(InventoryReservationResult::isSuccess);
        if(allProductsInStock)
        {
            orderRepo.save(order);
           String invResp = webClientBuilder.build().put()
                            .uri("http://inventory-service/api/inventory")
                            .bodyValue(skuCodes)
                            .retrieve()
                            .bodyToMono(String.class)
                            .block();
//            System.out.println("to-reduce "+invResp);
